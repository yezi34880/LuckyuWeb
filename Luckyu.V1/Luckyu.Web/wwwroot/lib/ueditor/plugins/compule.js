UE.compule = {
    ...
    getRadioOrCheckboxHtml: function ($control, type)
    {
        ...
        if ("0" == datasource)//数据字典
        {
            ...
            if ('radio' == type)
            {
                radios = '<span>@Html.Raw(BDictionary.GetRadiosByID("' + dictid + '".ToGuid(), "' + id + '", Business.Platform.Dictionary.OptionValueField.ID, "", "isflow=\'1\' type1=\'flow_radio\'' + eventArrs + '"))' + eventScripts + '</span>';
            }
            else if ('checkbox' == type)
            {
                radios = '<span>@Html.Raw(BDictionary.GetCheckboxsByID("' + dictid + '".ToGuid(), "' + id + '", Business.Platform.Dictionary.OptionValueField.ID, "", "isflow=\'1\' type1=\'flow_checkbox\'' + eventArrs + '"))' + eventScripts + '</span>';
            }
        ...
        else if ("1" == datasource)//自定义
        {
            ...
            for (var i = 0; i < customopts.length; i++)
            {
                ...
                radios += '<input type="' + type + '" name="' + id + '" id="' + id + '_' + i.toString() + '" value="' + value + '" style="vertical-align:middle;" isflow="1" type1="flow_' + type + '"' + eventArrs + '/>';
                radios += '<label for="' + id + '_' + i.toString() + '" style="vertical-align:middle;margin-right:3px;">' + title + '</label>';
            }
            radios += eventScripts;
            $control.after(radios);
            $control.remove();
        ...
        else if ("2" == datasource)//SQL
        {
            ...
            if ("radio" == type)
            {
                radios += '<span>@Html.Raw(new Business.Platform.WorkFlowForm().GetRadioFromSql(DBConnID, "' + sql.replaceAll('"', '\"') + '", "' + id + '", "", "isflow=\'1\' type1=\'flow_radio\'' + eventArrs + '"))' + eventScripts + '</span>';
            }
            else if ("checkbox" == type)
            {
                radios += '<span>@Html.Raw(new Business.Platform.WorkFlowForm().GetCheckboxFromSql(DBConnID, "' + sql.replaceAll('"', '\"') + '", "' + id + '", "", "isflow=\'1\' type1=\'flow_checkbox\'' + eventArrs + '"))' + eventScripts + '</span>';
            }
            $control.after(radios);
            $control.remove();
        }
    ...
    getSelectHtml: function ($control)
    {
        ...
        if ("0" == datasource)//数据字典
        {
            ...
            radios += '@Html.Raw(BDictionary.GetOptionsByID("' + dictid + '".ToGuid(), Business.Platform.Dictionary.OptionValueField.ID, ""))';
            radios += '</select>';
            var $radios = $(radios);
        ...
        else if ("1" == datasource)//自定义
        {
            ...
            for (var i = 0; i < customopts.length; i++)
            {
                ...
                radios += '<option value="' + value + '">' + title + '</option>';
            }
            radios += '</select>';
            var $radios = $(radios);
            $radios.attr("eventsid", $control.attr("eventsid"));
