    ...
    Class.prototype={
        ...
        getChecked: function(leafOnly, includeHalfChecked) {
            ...
            el.each(function(index,item) {
                ...
                var label=$(item).siblings(".eleTree-node-content-label").html();
        ...
        getHighlightNode: function(key) {
            ...
            var f = function (data) {
                data.forEach(function (val) {
                    var bool = false;
                    if (val[options.request.key] === obj[options.request.key]) {
                        bool = true;
                        for (var k in val) {
                            if (!obj[k] && k !== options.request.children) obj[k] = val[k];
                        }
                    }
                    if (!bool && val[options.request.children]) f(val[options.request.children]);
                })
