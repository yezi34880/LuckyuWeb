...
(function (factory) {
        ...
        styleUI: {
            ...
            Bootstrap: {
                ...
                colmenu: {
                    menu_widget: '',
                    input_checkbox: "",
                    filter_select: "layui-input",
                    filter_input: "layui-input",
        ...
        return this.each(function () {
            ...
            var sortkeys = ["shiftKey", "altKey", "ctrlKey"],
                ...
                addMulti = function (rowid, pos, irow, checked, uiclass) {
                    var v = "<input role=\"checkbox\"  lay-ignore type=\"checkbox\"" + " id=\"jqg_" + ts.p.id + "_" + rowid + "\" " + uiclass + " name=\"jqg_" + ts.p.id + "_" + rowid + "\"" + (checked ? "checked=\"checked\"" : "") + "/>",
                ...
                addJSONData = function (data, rcnt, more, adjust) {
                    ...
                    //if (!!data.rows)    //add by limin ,2020-02-05
                    //    ts.p.rawData = data.rows;
                    if (data instanceof Array) {  // 不分页 直接赋值
                        ts.p.rawData = data;
                    }
                    else if (!!data.rows) {
                        ts.p.rawData = data.rows;
                    ...
                    $.each(ts.p.colModel, function () {
                        ...
                        if (sorttype === "date" || sorttype === "datetime") {
                            if (this.formatter && typeof this.formatter === 'string' && this.formatter === 'date') {
                                ...
                                srcformat = newformat = this.datefmt || "Y-m-d H:i:s";
                            }
            ...
            if (this.p.multiselect) {
                this.p.colNames.unshift("<input role='checkbox' lay-ignore lay-filter='cb_" + this.p.id + "' id='cb_" + this.p.id + "' class='cbox' type='checkbox'/>");
    ...
    $.jgrid.extend({
        ...
        getRawRowData: function (rowId) {
            var $t = this[0];
            if (!$t || !$t.grid) {
                return null;
            }
            var rawData = $t.p.rawData;
            if (!rowId) {
                return rawData;
            }
            ...
            var ret = null;
            var keyName = $t.p.keyName;
            $(rawData).each(function () {
                for (var key in $(this)[0]) {
                    if (key == keyName && $(this)[0][key] == rowId) {
                        ret = $(this)[0];
                        break;
                    }
                }
            ...
            if (rdata) {
                ...
                this.each(function () {
                    ...
                    while (k < datalen) {
                        ...
                        if (gi) {
                            v = "<input role=\"checkbox\" lay-ignore type=\"checkbox\"" + " id=\"jqg_" + t.p.id + "_" + rowid + "\" " + msc + "/>";
        ...
        editCell: function (iRow, iCol, ed, event) {
            return this.each(function () {
                ...
                if (cm.editable === true && ed === true && !cc.hasClass("not-editable-cell") && (!$.isFunction($t.p.isCellEditable) || $t.p.isCellEditable.call($t, nm, iRow, iCol))) {
                    ...
                    var opt = $.extend({}, cm.editoptions || {}, { id: nm + "_" + iRow, name: nm, rowId: $t.rows[iRow].id, oper: 'edit' });
                ...
                if (edtrul.date === true) {
                    if (!(rqfield === false && $.jgrid.isEmpty(val))) {
                        if (cm[valref].formatoptions && cm[valref].formatoptions.newformat) {
                            ...
                            dft = cm[valref].datefmt || "Y-m-d H:i:s";
                        }
                ...
                $.each($t.p.colModel, function (ci) {
                    ...
                    if (this.search) {
                        ...
                        if (!$(elem).is("input")) {
                            $("td:eq(2)", stbl).hide();
                            if ($(elem).is("select")) {
                                $(elem).addClass("jqgrid-select");
                            }
                        } else {
                            $(elem).addClass(classes.srInput).attr('placeholder', "回车键执行搜索").attr("autocomplete", "off");
                        }
