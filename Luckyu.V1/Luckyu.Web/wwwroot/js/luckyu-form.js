    ...
    luckyu.form = {
        ...
        init: function () {
            $.fn.extend({
                 ...
                 * */
                initExt: function () {
                    $(this).find('input[lay-verify=number],div.xm-select').each(function () {
                        var self = $(this);
                        if (self.is("input")) {
                            self.numeric();
                        }
                        else {
                            var extType = self.attr("ext-type");
                            var multiple = self.attr("ext-multiple");
                            multiple = multiple === true;
                            switch (extType) {
                                case "datalocal":
                                    var localdata = self.attr("ext-localdata");
                                    self.initLocal({ data: JSON.parse(localdata), multiple: multiple });
                                    break;
                                case "dataitem":
                                    var code = self.attr("ext-code");
                                    self.initDataItem({ code: code, multiple: multiple });
                                    break;
                                case "datasource":
                                    var sourceUrl = self.attr("ext-sourceurl");
                                    var nameField = self.attr("ext-name");
                                    var valueField = self.attr("ext-value");
                                    self.initDataSource({
                                        url: sourceUrl,
                ...
                setVerify: function () {
                    var form = $(this);
                    luckyu.ajax.getSync("/BaseModule/Verify/GetVerifyFields", {}, function (res) {
                        var data = res.data;
                        if (!!data && data.length > 0) {
                            for (var i = 0; i < data.length; i++) {
                                var item = data[i];
                                var ele = form.find("#" + item.FieldName);
                                if (ele) {
                                    var label = ele.parent().prev();
                                    if (!!item.ShowName) {
                                        if (label.is("label") && label.hasClass("layui-form-label") && item.ShowName.length > 0) {
                                            label.html(item.ShowName);
                                        }
                                    }
                                    if (!!item.LayVerify) {
